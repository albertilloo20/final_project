...
## Instalando GIT
Para instalar git en nuestro ordenador tendremos que acceder a la pagina: https://git-scm.com/downloads e instalarlo dependiendo de nuestro sistema operativo.
...
Para bajarse el proyecto es necesario acceder a la terminal de ATOM, recordad que en el curso que os pase el profesor instala atom y varios plugins, uno de ellos es para usar la terminal en el ATOM (Esta en el curso seccion 1 clase 4).
...
Antes de nada, una vez os hayais bajado el proyecto con el comando `git clone` tendreis que instalar los nodos que he metido yo (jquery, bootstrap y popperjs), para ello una vez posicionados en la rama master o dev teneis que ejecutar el siguiente comando: `npm install`. Esto sera necesario realizarlo cada vez que actualicemos una libreria o importemos cualquier cosa en nuestro proyecto.

Para comprobar que nuestro proyecto funciona, que lo hemos clonado bien y todo eso, ejecutaremos el comando `ng serve` para compilar el proyecto, y para ver nuestra web en el navegador tendremos que poner la siguiente direccion: http://localhost:4200/.
...
## TRELLO
Trello es una pizarra para gestionar tareas, os recomiendo que useis esta aplicacion, de esta manera veremos que tareas hay
que hacer, que tareas se estan desarrollando y quien la esta desarrollando. 

Ya os pasare el enlace con la pizarra de momento solo registraros, el nombre de las ramas que creemos en git deberian de ser iguales al nombre de la tarea que haya creada en trello, de esta forma podemos ver el codigo de alguien que haya hecho una tarea y la haya pusheado sabiendo simplemente el nombre de esa tarea.
...
`git commit -m 'Mensaje a escribir'` -> esto commiteara nuestros archivos (por asi decirlo es un punto de restauracion).
El mensaje es totalmente obligatorio y deberia describir que se ha hecho con ese commit.
